...
import random
import time
import threading  # Importamos la librería threading (aunque no la usaremos explícitamente aquí con 'after')
...
class Interfaz(tk.Tk):
    def __init__(self):
        ...
        # --- Configuración del script anti-inactividad ---
        self._anti_inactividad_activo = True
        self._tiempo_espera_ai = 1 * 1000  # Convertir a milisegundos para after()
        self._movimiento_base_ai = 2
        self._incremento_movimiento_ai = 1
        self._maximo_incremento_ai = 15
        self._probabilidad_accion_ai = 0.1
        self._rango_clic_ai = 5
        self._incremento_actual_ai = 0
        self._teclas_aleatorias_ai = [
            win32con.VK_SHIFT,
            win32con.VK_CONTROL,
    ...
    def _simular_p(self, tecla):
        win32api.keybd_event(tecla, 0, 0, 0)
        time.sleep(random.uniform(0.1, 0.3))
        win32api.keybd_event(tecla, 0, win32con.KEYEVENTF_KEYUP, 0)

    def _simular_c(self, x, y, rango):
        _cx = x + random.randint(-rango, rango)
        _cy = y + random.randint(-rango, rango)
        win32api.SetCursorPos((_cx, _cy))
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, _cx, _cy, 0, 0)
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, _cx, _cy, 0, 0)
        win32api.SetCursorPos((x, y))

    def _ejecutar_(self):
        if self._anti_inactividad_activo:
            _x, _y = win32api.GetCursorPos()
            _movimiento_actual = self._movimiento_base_ai + self._incremento_actual_ai
            _dx = random.randint(-_movimiento_actual, _movimiento_actual)
            _dy = random.randint(-_movimiento_actual, _movimiento_actual)
            _nx = _x + _dx
            _ny = _y + _dy
            if _dx == 0 and _dy == 0:
                _nx += 1

            win32api.SetCursorPos((_nx, _ny))
            #print(f"a ({_nx}, {_ny})")

            if random.random() < self._probabilidad_accion_ai:
                _accion = random.randint(0, 1) # 0: Tecla, 1: Clic
                if _accion == 0:
                    _tecla = random.choice(self._teclas_aleatorias_ai)
                    #print(f"Anti-inactividad: Presionando tecla: {_tecla}")
                    self._simular_p(_tecla)
                else:
                    #print(f"Anti-inactividad: Simulando clic cerca de ({_x}, {_y})")
                    self._simular_c(_x, _y, self._rango_clic_ai)

            self._incremento_actual_ai = min(self._incremento_actual_ai + self._incremento_movimiento_ai, self._maximo_incremento_ai)
            self.after(self._tiempo_espera_ai, self._ejecutar_)

    def _programar_(self):
        self.after(self._tiempo_espera_ai, self._ejecutar_)
